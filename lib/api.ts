...
async function resolveRTDBCompanyKey(name: string) {
  try {
    ...
    const found = Object.keys(obj).find((k) => {
      const display = String(obj[k]?.companyDetails?.companyName || "")
        .toLowerCase()
        .trim();
...
export async function getCompanyLedgers(companyName: string) {
  try {
    ...
    const val = snap.val();
    const ledgers = Array.isArray(val)
      ? val
...
export async function getCompanyStocks(companyName: string) {
  try {
    ...
    const val = snap.val();
    const stocks = Array.isArray(val)
      ? val
      : val && typeof val === "object"
        ? Object.entries(val)
...
export async function getCompanyParties(companyName: string) {
  try {
    ...
    const val = snap.val();
    const parties = Array.isArray(val)
      ? val
      : val && typeof val === "object"
        ? Object.values(val)
        : [];
...
export async function getStocksWithBatches(companyName: string) {
  try {
    ...
    const stocksSnap = await rGet(
      rRef(rtdb, `companiesData/${companyKey}/stocks`),
    );
    const stocksVal = stocksSnap.val();
    const stocks = Array.isArray(stocksVal)
      ? stocksVal
    ...
    const batchesSnap = await rGet(
      rRef(rtdb, `stockBatchesByCompany/${companyKey}`),
    );
    const batchesObj = batchesSnap.val() || {};
    const batchDocs: any[] = Array.isArray(batchesObj)
      ? batchesObj
      : Object.values(batchesObj);
    ...
    for (const b of batchDocs) {
      ...
      batchMap[nameKey] = {
        batches: Array.isArray(b?.batches)
          ? b.batches
    ...
    const data = stocks.map((s: any) => {
      ...
      const info = batchMap[nameKey] || {
        batches: [],
        totalQuantity: 0,
      };
      const closingBalance =
        Number(s.$ClosingBalance ?? s.ClosingBalance ?? 0) || 0;
      return {
        ...s,
...
export async function getBatches(companyName: string, stockItem: string) {
  try {
    ...
    const snap = await rGet(
      rRef(rtdb, `stockBatchesByCompany/${companyKey}/${itemKey}`),
    );
    const v = snap.val() || {};
    const batches = Array.isArray(v?.batches)
      ? v.batches
...
export async function addBatches(payload: {
  ...
  try {
    ...
    const companyKey = await resolveRTDBCompanyKey(companyName);
    const itemKey = sanitizeDocId(stockItem);
    const byCompanyRef = rRef(
      rtdb,
      `stockBatchesByCompany/${companyKey}/${itemKey}`,
    );
    const existingSnap = await rGet(byCompanyRef);
    if (!existingSnap.exists()) {
      await rSet(byCompanyRef, {
        ...document,
...
export async function submitOrder(
  ...
  try {
    ...
    for (const item of items) {
      ...
      console.log(
        `✓ Updated batches for ${stockItem}, reduced by ${totalReduced}`,
      );
    }
